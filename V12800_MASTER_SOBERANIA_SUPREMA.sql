-- 
-- 👑 SQL MASTER V12800 - SOBERANIA SUPREMA (AMANDA ABREU)
-- OBJETIVO: Consolidação Final e Fix do Admin Hub (Denúncias Invisíveis).
-- FIX: Permissões de Inserção, RLS para Admin e Sincronização de Visibilidade.
--
...
-- ==========================================
-- 2. HARDENING DE TABELAS E REPARAÇÃO DE ÓRFÃOS
-- ==========================================

-- Garantir que a tabela de posts está saudável
...
-- Garantir que a tabela de denúncias tem todas as colunas de "Caixa Preta"
ALTER TABLE public.reports ADD COLUMN IF NOT EXISTS target_author_id UUID;
ALTER TABLE public.reports ADD COLUMN IF NOT EXISTS reported_content_text TEXT;

-- Limpeza de lixo técnico
DO $$ 
BEGIN 
    ...
    -- Cascatas de segurança
    ALTER TABLE IF EXISTS public.post_votes DROP CONSTRAINT IF EXISTS post_votes_post_id_fkey;
    ALTER TABLE IF EXISTS public.post_votes ADD CONSTRAINT post_votes_post_id_fkey FOREIGN KEY (post_id) REFERENCES public.posts(id) ON DELETE CASCADE;

    ALTER TABLE IF EXISTS public.reports DROP CONSTRAINT IF EXISTS reports_post_id_fkey;
    ALTER TABLE IF EXISTS public.reports ADD CONSTRAINT reports_post_id_fkey FOREIGN KEY (post_id) REFERENCES public.posts(id) ON DELETE CASCADE;
END $$;

-- ==========================================
-- 3. RECONSTRUÇÃO DA INTELIGÊNCIA (RAG V12800)
-- ==========================================
CREATE UNIQUE INDEX IF NOT EXISTS idx_knowledge_post_id ON public.knowledge_store ((metadata->>'post_id'));

CREATE OR REPLACE FUNCTION public.sync_post_to_ai_v12800()
RETURNS TRIGGER AS $$
BEGIN
...
DROP TRIGGER IF EXISTS trg_sync_post_to_ai_v12800 ON public.posts;
CREATE TRIGGER trg_sync_post_to_ai_v12800 AFTER INSERT OR UPDATE OR DELETE ON public.posts FOR EACH ROW EXECUTE FUNCTION public.sync_post_to_ai_v12800();

-- ==========================================
-- 4. RECONSTRUÇÃO DAS VIEWS (COM VISIBILIDADE TOTAL)
-- ==========================================
...
SELECT r.id, r.reason, r.status, r.created_at, r.reported_content_text, r.post_id, r.comment_id,
    jsonb_build_object('id', r.reporter_id, 'name', COALESCE(p_rep.username, 'Membro Anónimo')) as reporter,
    jsonb_build_object('id', r.target_author_id, 'name', COALESCE(p_tar.username, 'Alvo'), 'avatar', p_tar.avatar_url) as offender
...
-- ==========================================
-- 5. FUNÇÕES DE FEED E MODERAÇÃO
-- ==========================================

CREATE OR REPLACE FUNCTION public.get_sovereign_community_feed_v10000(p_limit INT DEFAULT 50, p_offset INT DEFAULT 0)
RETURNS TABLE (id UUID, author_id UUID, title TEXT, content TEXT, category TEXT, is_verified BOOLEAN, validation_status TEXT, created_at TIMESTAMPTZ, likes_count INT, useful_count INT, fake_count INT, reports_count INT, nobel_score INT, translations JSONB, author_data JSONB, comments_data JSONB) AS $$
BEGIN
    RETURN QUERY
    WITH current_votes AS (SELECT v.post_id, COUNT(*) FILTER (WHERE v.vote_type = 'like')::INT as l, COUNT(*) FILTER (WHERE v.vote_type = 'useful')::INT as u, COUNT(*) FILTER (WHERE v.vote_type = 'fake')::INT as f FROM public.post_votes v GROUP BY v.post_id),
    current_reports AS (SELECT r.post_id, COUNT(*)::INT as rep_count FROM public.reports r GROUP BY r.post_id)
    SELECT p.id, p.author_id, p.title, p.content, p.category, p.is_verified, p.validation_status, p.created_at, COALESCE(cv.l, 0), COALESCE(cv.u, 0), COALESCE(cv.f, 0), COALESCE(cr.rep_count, 0), COALESCE(ns.calculated_nobel_score, 0)::INT, p.translations,
        jsonb_build_object('name', prof.username, 'avatar_url', prof.avatar_url, 'level', prof.level, 'is_verified', prof.is_verified),
        COALESCE((SELECT jsonb_agg(jsonb_build_object('id', c.id, 'content', c.content, 'author_name', cp.username, 'created_at', c.created_at, 'likes', c.likes)) FROM public.comments c JOIN public.profiles cp ON c.author_id = cp.id WHERE c.post_id = p.id), '[]'::jsonb)
    FROM public.posts p JOIN public.profiles prof ON p.author_id = prof.id LEFT JOIN current_votes cv ON cv.post_id = p.id LEFT JOIN current_reports cr ON cr.post_id = p.id LEFT JOIN public.community_top_stories_nobel ns ON p.id = ns.id
    WHERE p.validation_status IS DISTINCT FROM 'blocked' ORDER BY COALESCE(ns.calculated_nobel_score, 0) DESC, p.created_at DESC LIMIT p_limit OFFSET p_offset;
END;
$$ LANGUAGE plpgsql SECURITY DEFINER;
...
-- ==========================================
-- 6. POLÍTICAS DE SOBERANIA (RLS FIX)
-- ==========================================
ALTER TABLE public.reports ENABLE ROW LEVEL SECURITY;

DROP POLICY IF EXISTS "Anyone can insert reports" ON public.reports;
CREATE POLICY "Anyone can insert reports" ON public.reports FOR INSERT TO authenticated WITH CHECK (true);

DROP POLICY IF EXISTS "Only Admin can see all reports" ON public.reports;
CREATE POLICY "Only Admin can see all reports" ON public.reports FOR SELECT TO authenticated 
