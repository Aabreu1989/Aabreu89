// Build trigger meta: MIRA v2.0.0-BETA-GOLD - LOCKDOWN v1beta - 2026-03-27 - SOVEREIGN NOBEL
...
const AppContent: React.FC = () => {
    ...
    const [adminTab, setAdminTab] = useState<'dashboard' | 'users' | 'moderation' | 'suggestions' | 'knowledge'>('dashboard');
    ...
    const fetchSavedPosts = async (userId: string) => {
        if (!userId || userId === 'undefined' || userId.length < 10) return;
        try {
        ...
        if (user?.id) {
            ...
            const channel = supabase
                ...
                .on('postgres_changes', { event: 'UPDATE', schema: 'public', table: 'profiles', filter: `id=eq.${user.id}` }, (payload) => {
                    ...
                    // 🔐 SECURITY SHIELD: Only update if it's the CURRENT user's profile
    ...
    const refreshData = async () => {
        ...
        try {
            // V2026.ULTRA: HIGH-LOAD BATCHING (50/Page)
            const PAGE_SIZE = 50;
    ...
    // V2026.ULTRA: AUTO-REFRESH ON ENTRY
    // Whenever the user enters the Community view, force a refresh of the first page.
    useEffect(() => {
        if (currentView === ViewType.COMMUNITY && user?.id) {
            console.log("MIRA_AUTO_REFRESH: Entering Community, syncing latest posts...");
            refreshData();
        }
    }, [currentView, user?.id]);
    ...
    const handleDeleteAccount = async () => {
        ...
        const confirmMsg = language === 'PT' 
            ? "⚠️ AÇÃO IRREVERSÍVEL: Deseja eliminar a sua conta e todos os seus dados permanentemente?" 
            : "⚠️ IRREVERSIBLE ACTION: Do you want to delete your account and all your data permanently?";
            
        if (!window.confirm(confirmMsg)) return;

        try {
            setIsRefreshing(true);
            // MIRA V2026.FINAL: Nuclear Purge RPC (RGPD Compliance)
            const { error } = await supabase.rpc('delete_own_account');
            
            if (error) {
                console.error("MIRA: Erro ao eliminar conta:", error);
                alert(language === 'PT' ? "Erro ao eliminar conta. Por favor, tente novamente." : "Error deleting account. Please try again.");
                return;
            }

            // Success: Nuclear logout and redirect
            console.log("MIRA: Conta eliminada com sucesso. Adeus.");
            await handleLogout();
        } catch (err) {
            console.error("MIRA: Catch error in delete account:", err);
            alert("Erro crítico ao processar o pedido.");
        } finally {
            setIsRefreshing(false);
        }
    };
    ...
    const renderView = () => {
        ...
        switch (currentView) {
            ...
            case ViewType.ASSISTANT: return <AssistantView language={language} user={user} onViewChange={handleViewChange} />;
                ...
                <div className={`min-h-screen ${[ViewType.ADMIN, ViewType.DASHBOARD].includes(currentView) ? 'bg-[#0A0A0A]' : 'bg-white'} flex flex-col font-sans overflow-hidden relative ${[ViewType.ADMIN, ViewType.DASHBOARD].includes(currentView) ? 'text-white' : 'text-slate-900'}`}>
                    ...
                    <div className="relative z-[1010]">
                        <TopBar
                            ...
                            isDark={currentView === ViewType.ADMIN || currentView === ViewType.DASHBOARD || (currentView !== ViewType.COMMUNITY && isSystemAdmin)}
                            onLogoClick={() => isSystemAdmin ? handleViewChange(ViewType.DASHBOARD) : handleViewChange(ViewType.HOME)}
                            onLogout={handleLogout}
                            ...
                            onProfileClick={() => handleViewChange(ViewType.PROFILE)}
                        />
                    </div>
                    ...
                    <div className="flex-1 flex overflow-hidden">
                        ...
                        <div className="hidden md:block w-24 h-full z-[1000] relative bg-white border-r border-slate-100">
                            <Navigation currentView={currentView} onViewChange={handleViewChange} language={language} />
                        </div>
                        <main className={`flex-1 overflow-hidden flex flex-col ${[ViewType.ADMIN, ViewType.DASHBOARD].includes(currentView) ? 'bg-[#0A0A0A]' : currentView === ViewType.COMMUNITY ? 'bg-white' : 'bg-[#F8F9FA]'} transition-colors duration-300`}>
                            <div className={`flex-1 flex flex-col ${currentView === ViewType.ASSISTANT ? 'overflow-hidden pb-[64px]' : 'overflow-y-auto overflow-x-hidden pb-20'}`}>
                        ...
                        {currentView !== ViewType.ASSISTANT && ![ViewType.ADMIN, ViewType.DASHBOARD].includes(currentView as any) && (
