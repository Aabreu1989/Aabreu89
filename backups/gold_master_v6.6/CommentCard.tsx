...
const timeAgo = (dateStr: string): string => {
    try {
        ...
        if (minutes < 60) return `h├í ${minutes} min`;
        const hours = Math.floor(minutes / 60);
        if (hours < 24) return `h├í ${hours} ${hours === 1 ? 'hora' : 'horas'}`;
        const days = Math.floor(hours / 24);
        if (days < 7) return `h├í ${days} ${days === 1 ? 'dia' : 'dias'}`;
        const weeks = Math.floor(days / 7);
        if (weeks < 4) return `h├í ${weeks} sem`;
        return dateStr;
    } catch {
        return dateStr;
    }
...
const CommentCard: React.FC<CommentCardProps> = ({ 
    ...
    return (
        <div className={`space-y-4 ${depth > 0 ? 'mt-4 ml-6 pl-4 border-l-2 border-white/10' : ''}`}>
                    ...
                    <div className="flex flex-col gap-1">
                        <div className="flex items-center justify-between mb-1">
                            ...
                            <button 
                                onClick={async () => {
                                    ...
                                    try {
                                        setIsReporting(true);
                                        await onReportComment(post.id, comment.id);
                                        showToast(language === 'PT' ? "Den├║ncia de coment├írio enviada com sucesso!" : "Comment report sent successfully!", "success");
                                    } catch (e) {
                                        showToast(language === 'PT' ? "Erro ao enviar den├║ncia." : "Error sending report.", "error");
                                    } finally {
                        ...
                        <div className="text-[14px] leading-relaxed text-slate-900 font-medium bg-white p-5 rounded-[30px] rounded-tl-none border border-slate-50 shadow-sm break-words [overflow-wrap:anywhere] [word-break:break-word]">
                            <TranslatedText 
                                ...
                                language={language} 
                                shouldTranslate={translatedPosts.has(comment.id)} 
                                translations={comment.translations}
                                onTranslationGenerated={async (res) => {
                                    try {
                                        await communityService.updateTranslation(comment.id, 'comment', language, res);
                                    } catch (e) {
                                        console.error("MIRA: Comment translation persistence failed", e);
                                    }
                                }}
                            />
                        </div>
                        ...
                        <div className="flex flex-wrap items-center gap-5 mt-3 ml-1">
                            ...
                            <button 
                                onClick={(e) => { e.stopPropagation(); onToggleTranslate(comment.id); }}
                                className={`p-3 rounded-2xl shadow-sm border transition-all ${translatedPosts.has(comment.id) ? 'bg-mira-orange text-white border-mira-orange scale-105' : 'bg-white text-slate-300 border-slate-50 hover:text-mira-orange'}`}
                                title={translatedPosts.has(comment.id) ? t('comm_view_original_btn', language) : t('comm_view_translate_btn', language)}
                            >
                                <Sparkles size={15} className={translatedPosts.has(comment.id) ? 'fill-white' : ''} />
                            </button>

                            {(isAdmin || comment.authorId === post.authorId || comment.authorName === post.authorName) && onDeleteComment && (
                                <button 
