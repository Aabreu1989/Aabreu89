-- 👑 MIRA V2026.GOLD: PATCH 2 (SOVEREIGN USER FOLLOWS & COUNTERS)
-- ---------------------------------------------------------------------------------------------
-- DESCRIÇÃO:
-- 1. get_sovereign_community_feed_v25 & get_sovereign_community_post_by_id_v25:
--    - Contadores de 'followers_count' e 'following_count' no objeto 'author' derivados dinamicamente
--      de public.user_follows (Soberania Relacional).
--    - Elimina a dependência de colunas estáticas em profiles.
-- ---------------------------------------------------------------------------------------------
...
BEGIN
    ...
    SELECT 
        ...
        jsonb_build_object(
            ...
            'followers_count', (SELECT count(*)::int FROM public.user_follows uf WHERE uf.following_id = prof.id),
            'following_count', (SELECT count(*)::int FROM public.user_follows uf WHERE uf.follower_id = prof.id)
        ) as author,
        COALESCE(
