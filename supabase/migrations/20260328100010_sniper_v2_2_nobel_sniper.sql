-- ============================================================
-- 🛡️ MIRA V2026.ULTRA: INTELIGÊNCIA SINTÉTICA HÍBRIDA (V2.2-NOBEL-SNIPER)
-- ------------------------------------------------------------
...
-- 5. RPC de Busca Híbrida v2.2 (FRESHNESS BOOST + INTEGRAL AUDIT)
...
CREATE TABLE IF NOT EXISTS public.expert_columns (
    ...
    embedding vector(768), -- Ajustado para Gemini text-embedding-004
...
-- [6] RPC: BUSCA HÍBRIDA SINTÉTICA V2.2-NOBEL-SNIPER (SNIPER CORE)
-- Esta função agora inclui o FRESHNESS BOOST (Conteúdo Recente = Maior Relevância)
...
BEGIN
    RETURN QUERY
    -- Fonte 1: Base de Conhecimento (Diretrizes CEO / AIMA)
    SELECT 
        k.content, k.source_type, k.category::TEXT, k.sovereignty_score,
        ((1 - (k.embedding <=> query_embedding)) * 0.7 + 
        ts_rank_cd(to_tsvector('portuguese', unaccent(k.content)), plainto_tsquery('portuguese', unaccent(query_text))) * 0.3)
        * (CASE WHEN k.created_at > (NOW() - INTERVAL '6 months') THEN 1.1 ELSE 1.0 END)::FLOAT as combined_score
    FROM public.knowledge_base k
    WHERE (1 - (k.embedding <=> query_embedding)) > match_threshold
    
    UNION ALL

    -- Fonte 2: Especialistas (Newsroom)
    SELECT 
        e.content, 'expert' as source_type, e.category::TEXT, 6000 as sovereignty_score,
        ((1 - (e.embedding <=> query_embedding)) * 0.7 + 
        ts_rank_cd(to_tsvector('portuguese', unaccent(e.content)), plainto_tsquery('portuguese', unaccent(query_text))) * 0.3)
        * (CASE WHEN e.created_at > (NOW() - INTERVAL '6 months') THEN 1.1 ELSE 1.0 END)::FLOAT as combined_score
    ...
    -- Fonte 3: Posts da Comunidade (Hacks da Tribo)
    -- BLINDAGEM TOTAL: Verified + Zero Reports + Engagement + Freshness
    SELECT 
        p.content, 'hack' as source_type, 'hacks_da_tribo' as category, 15000 as sovereignty_score,
        ((1 - (p.embedding <=> query_embedding)) * 0.7 + 
        ts_rank_cd(to_tsvector('portuguese', unaccent(p.content)), plainto_tsquery('portuguese', unaccent(query_text))) * 0.3)
        * (CASE WHEN p.created_at > (NOW() - INTERVAL '6 months') THEN 1.1 ELSE 1.0 END)::FLOAT as combined_score
    FROM public.posts p
    ...
    AND (SELECT count(*) FROM public.comments c WHERE c.post_id = p.id) > 0 
...
CREATE TRIGGER tr_sovereign_v2_0 BEFORE INSERT OR UPDATE ON public.knowledge_base FOR EACH ROW EXECUTE FUNCTION public.apply_sovereign_logic_v2_0();
