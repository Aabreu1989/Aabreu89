...
export default function PremiosView({ language: initialLanguage, onBack }: PremiosViewProps) {
  ...
  const formAnswers = {
    impact: `O MIRA gerou um impacto social mensurável significativo em 2026: poupou uma estimativa de ${metrics?.tempo_poupado_horas.toLocaleString()} horas de atrito burocrático aos cidadãos migrantes (modelo ponderado MIRA), prestou apoio em ${metrics?.processos_ajudados.toLocaleString()} operações burocráticas (minutas geradas e simulações fiscais/laborais) e manteve o índice de transparência em ${metrics?.indice_transparencia}%. Contamos com ${metrics?.usuarios_ativos_mensais.toLocaleString()} perfis registados no ecossistema e uma taxa de resolução na orientação de trâmites de ${metrics?.taxa_resolucao_sucesso}%.`,
    ...
    scalability: `Altamente escalável: a infraestrutura digital é extremamente eficiente, integrando ${metrics?.usuarios_ativos_mensais.toLocaleString()} perfis no ecossistema com custos de servidores infraestruturais mínimos e expansível de forma modular para qualquer estado-membro da UE.`,
