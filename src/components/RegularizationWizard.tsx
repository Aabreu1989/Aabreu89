...
interface CrossNavButtonConfig {
    id: string;
    labelKey: string;
    descKey: string;
    icon: React.ReactNode;
...
const getCrossNavButtons = (sit?: string, purpose?: string): CrossNavButtonConfig[] => {
    const btnJobsWork: CrossNavButtonConfig = {
        id: 'jobs_work',
        labelKey: 'wiz_cross_nav_jobs_work',
        descKey: 'wiz_cross_nav_jobs_desc',
        icon: <Briefcase size={18} className="text-emerald-500" />,
        view: ViewType.JOBS,
        gradient: 'hover:border-emerald-300 hover:bg-emerald-50/50'
    };

    const btnJobs: CrossNavButtonConfig = {
        id: 'jobs',
    ...
    const btnSimulators: CrossNavButtonConfig = {
        id: 'simulators',
        labelKey: 'wiz_cross_nav_simulators',
        descKey: 'wiz_cross_nav_simulators_desc',
        icon: <Calculator size={18} className="text-cyan-500" />,
        view: ViewType.SIMULATORS,
        gradient: 'hover:border-cyan-300 hover:bg-cyan-50/50'
    };

    const btnCoursesHub: CrossNavButtonConfig = {
        id: 'courses_hub',
        ...
        gradient: 'hover:border-blue-300 hover:bg-blue-50/50'
    };

    const btnCoursesShort: CrossNavButtonConfig = {
        id: 'courses_short',
        labelKey: 'wiz_cross_nav_courses_short',
        descKey: 'wiz_cross_nav_courses_desc',
        icon: <GraduationCap size={18} className="text-blue-500" />,
        view: ViewType.LEARNING,
        gradient: 'hover:border-blue-300 hover:bg-blue-50/50'
    ...
    const btnCommunity: CrossNavButtonConfig = {
        id: 'community',
        labelKey: 'wiz_cross_nav_community',
        descKey: 'wiz_cross_nav_community_desc',
        icon: <Users size={18} className="text-purple-500" />,
        view: ViewType.COMMUNITY,
        gradient: 'hover:border-purple-300 hover:bg-purple-50/50'
    };

    const btnServices: CrossNavButtonConfig = {
        id: 'services',
        ...
        gradient: 'hover:border-amber-300 hover:bg-amber-50/50'
    };

    // 8 - VISTOS DE PEDIDO DE PROTEÇÃO ASILO: VAGAS DE EMPREGO, CURSOS, SERVIÇOS, COMUNIDADE, SIMULAÇÕES
    if (sit === 'asylum' || purpose === 'humanitarian') {
        return [btnJobs, btnCoursesShort, btnServices, btnCommunity, btnSimulators];
    }

    // 7 - VISTOS DE REFORMA: SIMULAÇÕES, SERVIÇOS, COMUNIDADE
    if (sit === 'retirement' || purpose === 'visa_d7') {
        return [btnSimulators, btnServices, btnCommunity];
    }

    // 6 - VISTOS DE REAGRUPAMENTO: VAGAS DE EMPREGO, CENTRAL DE CURSOS, SERVIÇOS
    if (sit === 'family' || purpose === 'art122') {
        return [btnJobs, btnCoursesHub, btnServices];
    }

    // 5 - VISTOS DE VIA VERDE: VAGAS DE EMPREGO, COMUNIDADE
    if (sit === 'via_verde') {
        return [btnJobs, btnCommunity];
    }

    // 4 - VISTOS DE NÓMADA DIGITAL: SIMULAÇÕES, COMUNIDADE
    if (purpose === 'art90a') {
        return [btnSimulators, btnCommunity];
    }

    // 3 - VISTOS DE EMPREENDEDOR: SIMULAÇÕES, COMUNIDADE
    if (purpose === 'art89') {
        return [btnSimulators, btnCommunity];
    }

    // 2 - VISTOS DE ESTUDANTE: CENTRAL DE CURSOS, COMUNIDADE, VAGAS DE EMPREGO
    if (sit === 'student' || purpose === 'visa_d4') {
        return [btnCoursesHub, btnCommunity, btnJobs];
    }

    // 1 - VISTOS DE TRABALHO: VAGAS DE TRABALHO, SIMULAÇÕES
                ...
                <div className="p-5 space-y-5 pb-32">
                    ...
                    {step === 4 && (
                        <div className="space-y-6 animate-in slide-in-from-bottom-4 duration-500">
                            ...
                            {onViewChange && (
                                <div className="bg-white border border-slate-100 rounded-[2.25rem] p-6 shadow-sm space-y-4 animate-in slide-in-from-bottom-3 duration-500">
                                    ...
                                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 pt-1">
                                        {getCrossNavButtons(answers.situation, answers.purpose).map((btn, idx) => (
                                            <button
                                                key={btn.id + '_' + idx}
                                                onClick={() => {
                                                    audioService.playClick();
                                                ...
                                                className={`group flex items-center justify-between p-4 bg-slate-50 border border-slate-100 rounded-2xl transition-all duration-300 hover:shadow-md hover:border-orange-300 active:scale-[0.98] ${btn.gradient}`}
                                            >
