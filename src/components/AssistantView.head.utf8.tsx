        ...
        <div className="bg-white/90 backdrop-blur-md border-b border-slate-100 px-3 sm:px-5 py-3 sm:py-4 flex items-center gap-3 sm:gap-4 z-[200] sticky top-0 shrink-0">
            ...
            <div className="flex items-center gap-3 sm:gap-4 flex-1 cursor-pointer group min-w-0" onClick={onShowAvatar}>
                <div className="relative shrink-0">
                    <div className="w-9 h-9 sm:w-11 sm:h-11 bg-white rounded-xl sm:rounded-2xl p-0.5 shadow-lg border border-slate-100 transition-transform group-hover:scale-105 duration-500 overflow-hidden flex items-center justify-center">
                        <img src={MIRA_PHOTO_URL} alt="MIRA Avatar" className="w-full h-full object-cover" />
                    </div>
                </div>
                <div className="flex flex-col min-w-0">
                    ...
                    <div className="flex items-center gap-1.5 px-3 py-1 bg-gradient-to-r from-emerald-50 to-emerald-100 rounded-full border border-emerald-200 w-fit group-hover:scale-105 transition-transform duration-300">
                      <div className="w-1.5 h-1.5 rounded-full bg-emerald-500 animate-pulse"></div>
                      <span className="text-[8px] sm:text-[9px] font-black text-emerald-700 uppercase tracking-[0.1em]">Sovereign Active</span>
                    </div>
                </div>
            </div>
...
const MiraChatMessage = React.memo(({ 
  ...
  return (
    <div className={`flex w-full mb-8 px-2 sm:px-5 ${isUser ? 'justify-end' : 'justify-start'} animate-in fade-in slide-in-from-bottom-4 duration-700`}>
      <div className={`max-w-[92%] sm:max-w-[75%] relative group transition-all duration-500
        ${isUser 
            ? 'px-6 py-4 bg-mira-orange text-white rounded-[2rem] rounded-tr-none shadow-xl' 
            : 'px-7 py-5 bg-[#0066FF] text-white rounded-[2rem] rounded-tl-none border-l-[4px] border-[#FF8C00] shadow-xl border border-white/5'}`}>

          {!isUser && (
            <div className="flex items-center gap-3 mb-4">
               <div className="px-3 py-1 bg-white/10 backdrop-blur-sm rounded-lg text-[8px] font-black uppercase tracking-[0.2em] text-emerald-400 border border-emerald-400/30 flex items-center gap-2 group-hover:border-emerald-400/60 transition-colors">
                <ShieldCheck size={10} className="text-mira-orange animate-pulse" />
                {msg.category?.includes('Soberano+') ? 'DIAMOND REALTIME' : 'DIAMOND SOVEREIGN'}
               </div>
               {isSovereign && (
...
const AssistantView = ({ language, onViewChange, user }: AssistantViewProps) => {
  ...
  const handleClearChat = async () => {
    if (window.confirm(t('comm_delete_post_q', language) || "Deseja eliminar esta conversa?") ) {
      ...
      } catch (e) { console.error("MIRA DB Clear Fail:", e); }
        ...
        <div className="fixed inset-0 z-[1000] bg-[#001F3F]/95 backdrop-blur-3xl flex items-center justify-center cursor-pointer p-0" onClick={() => setShowAvatarModal(false)}>
            ...
            <div className="w-full h-full p-6 flex items-center justify-center" onClick={e => e.stopPropagation()}>
                <div className="max-w-lg w-full aspect-square bg-slate-900 rounded-[3rem] p-1 shadow-2xl overflow-hidden border border-white/10 animate-in zoom-in-95 duration-500">
                    <img src={MIRA_PHOTO_URL} alt="MIRA" className="w-full h-full object-cover rounded-[2.8rem]" />
                </div>
            </div>
        </div>
      )}
